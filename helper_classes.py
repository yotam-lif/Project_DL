...
class TF:
    def __init__(self, arr):
        ...
        self.disorder_score = torch.FloatTensor(arr[1][:self.seq_len])
        self.molw = torch.FloatTensor(arr[2][:self.seq_len])
        self.res_molw = torch.FloatTensor(arr[3][:self.seq_len])
        self.pka = torch.FloatTensor(arr[4][:self.seq_len])
        self.pkb = torch.FloatTensor(arr[5][:self.seq_len])
        self.iso = torch.FloatTensor(arr[6][:self.seq_len])
        ...
        for i in range(self.seq_len):
            ...
            if ind >= 0:
                self.seq[i][ind - 1] = 1
        self.seq = torch.FloatTensor(self.seq)

        self.DBD = np.zeros(DBD_nums)
        dbd_enc = int(arr[0][-1]) - 1
        self.DBD[dbd_enc] = 1
        self.DBD = torch.FloatTensor(self.DBD)
...
class DNA_fragment:
    def __init__(self, arr):
        ...
        self.pka = torch.FloatTensor(arr[1][0:self.seq_len])
        self.molw = torch.FloatTensor(arr[2][0:self.seq_len])
        self.signal = torch.FloatTensor(arr[3][0:self.seq_len])
...
class Custom_MSE_Loss(nn.Module):
    ...
    def forward(self, inputs, targets):
        mse = torch.pow(inputs - targets, 2)
        coeff = 1 + self.c * targets
        loss = coeff * mse
        print(loss)
