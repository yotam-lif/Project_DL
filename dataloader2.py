...
import numpy as np
import h5py
from helper_classes2 import TF as TF
from helper_classes2 import Chromosome as Chromosome
...
class TFDNA_ds(Dataset):
    def __init__(self, DNA_path, TF_path, TFnum, seq_length=300, sliding_window_step=1):
        """
    ...
    def __len__(self):
        return int(np.sum(self.num_ind))

    def __getitem__(self, idx):
        TF_num, chrom_num, loc_on_chrom = self.get_TF_chrom_and_loc_from_ind(
            idx, chrom_lengths=self.chrom_lengths, seq_length=self.seq_length,
        ...
        chrom = Chromosome(
            chrom_file[chrom_key], chrom_num, TF_num, seq_length=self.seq_length,
            sliding_window_step=self.sliding_window_step)
    ...
    def get_chrom_lengths(self, num_chroms=16, file_name='signal_159_TFs/DNA_data_fragments_', TF=1):
        # returns an array with the chromosomes lengths for indexing purposes
        chrom_lengths = np.zeros(num_chroms)
        for i in range(num_chroms):
            path = file_name + str(i + 1) + '.h5'
            file = h5py.File(path, 'r')
            chrom_key = 'DNA_data_frags_00' + str(TF) + '_chrom' + str(i + 1)
            chrom = Chromosome(file[chrom_key], i + 1, 1)
            chrom_lengths[i] = chrom.chrom_len
        return chrom_lengths

    def get_index_from_TF_and_chromosome(self, ind_on_chrom, TF_num, chrom_num, chrom_lengths, seq_length=300,
        ...
        ind =  DNA_tot_ind_per_TF + cum_chrom_ind[chrom_num - 1] + ind_on_chrom
        return ind
